...
export default function OrdersPage() {
  ...
  const filteredOrders = useMemo(() => {
    ...
    return orders.filter((order) => {
      if (view === 'ALL' && order.status === 'CANCELLED') {
        return false;
      }
      if (view !== 'ALL' && order.status !== view) {
        return false;
      }
      if (!normalizedSearch) {
        return true;
    ...
    <div className="space-y-6">
      ...
      <div className="grid gap-4 lg:grid-cols-[1fr_auto]">
        ...
        <div className="flex gap-2 overflow-x-auto rounded-3xl bg-white p-3 shadow-sm">
          {orderViews.map((item) => {
            const count = item.value === 'CANCELLED'
              ? cancelledCount
              : item.value === 'ALL'
      ...
      {filteredOrders.length ? (
              ...
              <div key={order.id} className={`rounded-3xl border bg-white p-6 shadow-sm ${order.status === 'CANCELLED' ? 'border-rose-200 bg-rose-50/40' : 'border-slate-200'}`}>
                <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                  ...
                  <div className="flex min-w-0 flex-col items-start gap-3 text-slate-700 sm:items-end">
                    ...
                    <p className="text-xl font-semibold text-slate-900">{formatMoney(order.totalAmount)}</p>
                        ...
                        {order.items.map((item) => (
                          <div key={item.id} className="flex flex-col gap-2 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
                            <div className="min-w-0">
                              ...
                              <p className="text-sm text-slate-600">{Number(item.quantityKg || 0).toFixed(2)} kg × {formatMoney(item.pricePerKg)}</p>
                            </div>
                            <p className="text-sm font-semibold text-slate-900">{formatMoney(item.subtotal)}</p>
                          </div>
                        ))}
                      </div>
                    </div>
        ...
        <div className="rounded-3xl bg-white p-8 text-center shadow-sm">
          <p className="text-slate-600">{view === 'CANCELLED' ? 'No cancelled orders.' : 'No orders found.'}</p>
        </div>
      )}
      ...
      {deleteTarget && (
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl">
            <h2 className="text-xl font-bold text-slate-900">⚠️ Permanently Delete Completed Order?</h2>
            ...
            <p className="text-sm text-slate-700">Total: <strong>{formatMoney(deleteTarget.totalAmount)}</strong></p>
      ...
      {cancelTarget && (
        <div className="fixed inset-0 z-30 flex items-center justify-center bg-slate-900/40 p-4">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-xl">
            ...
            <p className="text-sm text-slate-700">Total: <strong>{formatMoney(cancelTarget.totalAmount)}</strong></p>
